...
def game_init(visitid, secure_token, sn, secure_cookie, campaignid, txnid):
    ...
    burp0_headers = {"Content-Type": "application/json", "Connection": "Keep-Alive", "User-Agent": "okhttp/3.14.4",
                     ...
                     "X-User-Agent": "Mozilla/5.0 (Linux; Android 8.1.0; T5524 Build/OPM7.181205.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/84.0.4147.125 Mobile Safari/537.36 FKUA/Retail/1190707/Android/Mobile (Smartron/T5524/"+"{}".format(visitid)+")",
                     "sn": "{}".format(sn),
...
def game_start(campaignid, tokenid):
    game_start_url = "https://2.games3p.api.flipkart.net:443/games-external-freefall/GameStart"
...
def game_complete(campaignid, tokenid):
    gc_url = "https://2.games3p.api.flipkart.net:443/games-external-freefall/GameComplete"
    gc_headers = {"Content-Type": "application/json", "Connection": "Keep-Alive", "Accept-Encoding": "gzip",
                  "User-Agent": "okhttp/3.14.4"}
    gc_json = {"campaignId": "{}".format(campaignid), "score": 704, "timeSpent": 244,
               "tokenId": "{}".format(tokenid)}
